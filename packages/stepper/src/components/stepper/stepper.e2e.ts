...
test.describe('ip-stepper', () => {
  ...
  test('should render and display the initial step correctly', async ({
    page,
  }) => {
    await page.evaluate(() => {
    ...
    const stepIndicator = page
      .locator('ip-stepper')
      .locator('.step-indicator span');
    await expect(stepIndicator).toHaveText('Step 1 / 3');
  });

  test('should update the step when clicking continue button', async ({
    page,
  }) => {
    await page.evaluate(() => {
      document.body.innerHTML = '<ip-stepper steps="3"></ip-stepper>';
  ...
  test('should show back button when not on the first step', async ({
    page,
  }) => {
    await page.evaluate(() => {
      document.body.innerHTML = '<ip-stepper steps="3"></ip-stepper>';
  ...
  test('should render step content correctly', async ({ page }) => {
    ...
    const stepContent = page.locator('ip-stepper').locator('.step-content');
